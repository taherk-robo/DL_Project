...
# #         self.gn7 = nn.GroupNorm(num_groups=self.num_groups, num_channels=1024)
...
# #         self.gn9 = nn.GroupNorm(num_groups=self.num_groups, num_channels=256)
# #         self.conv7 = nn.Conv1d(256, 3, 1)

# #     def forward(self, x):
# #         # Encoder
# #         x = F.relu(self.gn1(self.conv1(x)))  # [batch_size, 64, num_points]
# #         x = F.relu(self.gn2(self.conv2(x)))  # [batch_size, 128, num_points]
# #         x = F.relu(self.gn3(self.conv3(x)))  # [batch_size, 256, num_points]
# #         x = F.relu(self.gn4(self.conv4(x)))  # [batch_size, 512, num_points]
# #         x = torch.max(x, 2, keepdim=True)[0]  # [batch_size, 512, 1]
# #         x = x.view(-1, 512)  # [batch_size, 512]
# #         x = F.relu(self.gn5(self.fc1(x)))  # [batch_size, 256]

# #         # Decoder
# #         x = F.relu(self.gn6(self.fc2(x)))  # [batch_size, 512]
# #         x = F.relu(self.gn7(self.fc3(x)))  # [batch_size, 1024]
# #         x = x.unsqueeze(2)  # [batch_size, 1024, 1]
# #         x = F.relu(self.gn8(self.conv5(x)))  # [batch_size, 512, 1]
# #         x = F.relu(self.gn9(self.conv6(x)))  # [batch_size, 256, 1]
# #         x = self.conv7(x)  # [batch_size, 3, 1]
# #         x = x.squeeze(2)  # [batch_size, 3]
# #         return x
...
# #         # Do not center or normalize the point cloud to preserve global positions
...
# #         loss_cd = chamfer_dist(output, target)
# #         return loss_cd
...
# #         all_reconstructions = []
# #         with torch.no_grad():
# #             for data, target in ddl_problem.train_loaders[i]:
# #                 data = data.to(device)
# #                 output = model(data)
# #                 output = output.cpu().numpy()  # [batch_size, 3]
...
# #         # Configuration
# #         conf = {
# #             "output_metadir": "path/to/output_directory",
# #             "name": "3d_map_DiNNO",
...
# #             "train_batch_size": 16,
# #             "val_batch_size": 16,
# #             "data_dir": "path/to/your/KITTI/dataset",
# #             "model": {
...
# #                 "linear_width": 64
# #             },
# #             "loss": "Chamfer",  # Using Chamfer Distance
...
# # Import Chamfer Distance from torchmetrics
# from torchmetrics.functional import pairwise_point_cloud_distance

# # Define the Chamfer Distance function
# def chamfer_distance(x, y):
#     # x and y are of shape [batch_size, num_points, 3]
#     # Returns the mean Chamfer Distance between the point clouds in the batch
#     return pairwise_point_cloud_distance(x, y, reduction='mean', metric='chamfer')
...
#         self.fc2 = nn.Linear(256, 512)
#         self.gn6 = nn.GroupNorm(num_groups=self.num_groups, num_channels=512)
#         self.fc3 = nn.Linear(512, 1024 * 3)  # Output flattened point cloud
#         self.gn7 = nn.GroupNorm(num_groups=self.num_groups, num_channels=1024 * 3)
...
#         x = F.relu(self.gn3(self.conv3(x)))  # [batch_size, 256, num_points]
#         x = F.relu(self.gn4(self.conv4(x)))  # [batch_size, 512, num_points]
#         x = torch.max(x, 2, keepdim=False)[0]  # [batch_size, 512]
#         x = F.relu(self.gn5(self.fc1(x)))  # [batch_size, 256]

#         # Decoder
#         x = F.relu(self.gn6(self.fc2(x)))  # [batch_size, 512]
#         x = self.fc3(x)  # [batch_size, 1024 * 3]
#         x = x.view(-1, 1024, 3)  # [batch_size, 1024, 3]
#         x = x.permute(0, 2, 1)  # [batch_size, 3, 1024]
#         return x
...
#         data, target = data.to(self.device), target.to(self.device)
#         output = model(data)
#         # output and target are of shape [batch_size, 3, num_points]
#         output = output.permute(0, 2, 1)  # [batch_size, num_points, 3]
#         target = target.permute(0, 2, 1)
#         loss_cd = chamfer_distance(output, target)
#         return loss_cd
...
#             "val_batch_size": 16,
#             # "data_dir": "path/to/your/KITTI/dataset",
#             "data_dir": "E:/Documents/Masters_Courses/ESE 5460/final_project_env/DL_Project/your_KITTI_data",
#             "model": {
#                 "in_channels": 3,  # Updated for PointNet
#                 "out_channels": 3,
#                 "init_features": 3,
...
#                 "linear_width": 64
#             },
#             "loss": "Chamfer",  # Using Chamfer Distance via torchmetrics
#             "use_cuda": torch.cuda.is_available(),
#             "individual_training": {
#                 "train_solo": False,
#                 "optimizer": "adam",
...
# # Define the custom Chamfer Distance function
...
#         output = output.permute(0, 2, 1)  # [batch_size, num_points, 3]
#         target = target.permute(0, 2, 1)
#         loss_cd = chamfer_distance_function(output, target)
#         return loss_cd
...
#             "verbose": True,
#             "graph": {
#                 "type": "fully_connected",  # Options: "fully_connected", "cycle", "ring", "star", "erdos_renyi"
#                 "num_nodes": 2,  # Adjust the number of nodes as needed
#                 "p": 0.3,
#                 "gen_attempts": 100
#             },
...
# Define Custom Dataset for Point Clouds
class PointCloudDataset(Dataset):
    def __init__(self, point_cloud, num_points=1024, num_samples=10000, augment=False):
        """
        Args:
            point_cloud: Numpy array of shape (N, 3).
            num_points: Number of points to sample from each point cloud.
...
if __name__ == "__main__":
    try:
        ...
        conf = {
            ...
            "train_batch_size": 128,  # Adjusted batch size to practical value
            "val_batch_size": 128,  # Adjusted batch size to practical value
            "data_dir": "E:/Documents/Masters_Courses/ESE 5460/final_project_env/DL_Project/2011_09_28_drive_0035_sync/2011_09_28/2011_09_28_drive_0035_sync/velodyne_points/data",
            "model": {
                "in_channels": 3,  # Updated for PointNet
                "out_channels": 3,
