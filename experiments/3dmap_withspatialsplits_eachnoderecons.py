...
# def reconstruct_and_align_map(ddl_problem, device, output_dir):
#     """
...
#         device: Torch device.
#         output_dir: Directory to save individual node reconstructions.
...
#         # **New Addition: Save Individual Node Reconstruction**
#         node_pcd_path = os.path.join(output_dir, f"reconstructed_map_node_{i}.pcd")
#         o3d.io.write_point_cloud(node_pcd_path, pcd)
#         print(f"Node {i}: Reconstructed map saved at {node_pcd_path}")
...
#             # **New Addition: Reconstruct and Save Individual Node Maps**
#             print("Reconstructing and saving individual node maps...")
#             # The 'reconstruct_and_align_map' function has been modified to save individual node reconstructions
#             # Pass the output directory to save the individual reconstructions
#             global_map = reconstruct_and_align_map(ddl_problem, DEVICE, conf["output_metadir"])
...
class PointNetAutoencoder(nn.Module):
    def __init__(self, num_points=1024):
        ...
        # Encoder
        self.conv1 = nn.Conv1d(3, 64, 1)
        self.gn1 = nn.GroupNorm(num_groups=8, num_channels=64)  # Replaced BatchNorm1d
        self.conv2 = nn.Conv1d(64, 128, 1)
        self.gn2 = nn.GroupNorm(num_groups=8, num_channels=128)  # Replaced BatchNorm1d
        self.conv3 = nn.Conv1d(128, 1024, 1)
        self.gn3 = nn.GroupNorm(num_groups=8, num_channels=1024)  # Replaced BatchNorm1d
        # self.conv4 = nn.Conv1d(1024, 2048, 1)  # Uncomment if increasing model complexity
        # self.bn4 = nn.BatchNorm1d(2048)
        self.fc1 = nn.Linear(1024, 512)
        self.gn4 = nn.GroupNorm(num_groups=8, num_channels=512)  # Replaced BatchNorm1d
        self.fc2 = nn.Linear(512, 256)
        self.gn5 = nn.GroupNorm(num_groups=8, num_channels=256)  # Replaced BatchNorm1d
        # Decoder
        self.fc3 = nn.Linear(256, 512)
        self.gn6 = nn.GroupNorm(num_groups=8, num_channels=512)  # Replaced BatchNorm1d
        self.fc4 = nn.Linear(512, 3 * self.num_points)
...
class DDLProblem:
    def __init__(self, models, N, conf, train_subsets, val_set):
        ...
        # Initialize communication graph
        self.graph = generate_from_conf(conf["graph"])[1]
        # Initialize data loaders for each node with drop_last=True
        self.train_loaders = [
            DataLoader(
                train_subsets[i], 
        ...
        # Validation loader can keep drop_last=False
...
def reconstruct_and_align_map(ddl_problem, device, output_dir):
    ...
    for i in range(ddl_problem.N):
        ...
        if not all_reconstructions:
            print(f"Node {i}: No reconstructions available.")
            continue
        reconstructed_points = np.concatenate(all_reconstructions, axis=0)
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(reconstructed_points)
    ...
    if not reconstructed_pcds:
        print("No node reconstructions available to aggregate.")
        return None
...
if __name__ == "__main__":
    ...
    if not all_points:
        print("No point cloud data found. Please check the 'data_dir' path.")
        sys.exit(1)
    ...
    if conf["individual_training"]["train_solo"]:
        ...
        if metrics is not None:
            ...
            # Reconstruct and save individual node maps
            print("Reconstructing and saving individual node maps...")
            global_map = reconstruct_and_align_map(ddl_problem, DEVICE, conf["output_metadir"])
            if global_map is not None:
                ...
                print("Global map reconstruction failed due to missing node reconstructions.")
